...
bivar_map_df_rural <- bivar_map_df %>% 
  left_join(pfas_mod_data[, c("geoid", "urban_rural")]) %>% 
  mutate(bi_class = case_when(urban_rural == "Urban" ~ NA,
                              TRUE ~ bi_class))
...
beepr::beep(10)
ggsave(paste0("output/figures/bivariate_SER_PFAS_plot",Sys.Date(),".png"), height = 20, width = 35)
beepr::beep(10)
